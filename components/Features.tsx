...
const Features: React.FC = () => {
  ...
  return (
    <section id="features" className="py-24 px-4 md:px-8 bg-[#0a0a0a]">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16">
          <span className="text-[#ff4d00] font-bold uppercase tracking-[0.3em] text-sm block mb-4">Nuestros Programas</span>
          <h2 className="text-5xl md:text-6xl font-black font-heading tracking-tight max-w-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((feature, idx) => (
            <div 
              key={idx} 
              className="group relative h-[500px] overflow-hidden rounded-[40px] border border-white/5 bg-zinc-900/50 hover:border-[#ff4d00]/30 transition-all cursor-pointer"
            >
              <img 
                src={feature.image} 
                alt={feature.title} 
              ...
              <div className="absolute bottom-0 left-0 p-8 w-full">
                <div className="bg-black/50 backdrop-blur-md w-fit p-4 rounded-2xl mb-6 group-hover:bg-[#ff4d00]/20 transition-colors">
                  {feature.icon}
                </div>
                <h3 className="text-2xl font-bold mb-3 group-hover:text-[#ff4d00] transition-colors">{feature.title}</h3>
                <p className="text-gray-400 group-hover:text-white transition-colors">{feature.description}</p>
                
                <div className="mt-8 overflow-hidden h-0 group-hover:h-8 transition-all duration-300">
                  <button className="text-[#ff4d00] font-bold flex items-center group/btn">
                    Leer Más 
