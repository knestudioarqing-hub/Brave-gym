...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className={`fixed top-12 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-black/90 backdrop-blur-md py-3' : 'bg-transparent py-6'}`}>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-black/95 absolute top-full left-0 right-0 h-screen p-6 flex flex-col space-y-6 animate-in slide-in-from-top duration-300">
          {navLinks.map((link) => (
            <a key={link.name} href={link.href} className="text-2xl font-bold text-white border-b border-white/10 pb-2">
              {link.name}
            </a>
          ))}
          <button className="bg-[#ff4d00] text-white py-4 rounded-xl text-lg font-bold">
            Únete Ahora
          </button>
        </div>
      )}
